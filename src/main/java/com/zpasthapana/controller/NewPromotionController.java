...
@RestController
@RequestMapping("new-promotions")
public class NewPromotionController {

	@Autowired
	NewPromotionService newPromotionService; 
	
	@PostMapping(consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
	public ResponseEntity<ResponseMessage> addNewPromotion(@ModelAttribute NewPromotionRequest request) {
		newPromotionService.addNewPromotion(request);
		return new ResponseEntity<ResponseMessage>(ResponseMessage.builder().status(HttpStatus.CREATED).build(),
				HttpStatus.CREATED);
	}
	
	@GetMapping
	public ResponseEntity<List<NewPromotion>> getAllNewPromotion() {
		return new ResponseEntity<List<NewPromotion>>(newPromotionService.getAllNewPromotion(),
				HttpStatus.OK);
	}
