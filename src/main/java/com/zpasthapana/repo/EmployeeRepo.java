...
public interface EmployeeRepo extends JpaRepository<Employee, Long>{
	...
	@Query(value = "select e from Employee e where e.retierment.retirementDate < :retirementDate and (:zpId is null or e.employeeWorkLocation.zpId = :zpId) "
	...
	@Query(value = "select e from Employee e left join e.retierment r where e.retierment IS NULL or r.retirementDate > :retirementDate and (:zpId is null or e.employeeWorkLocation.zpId = :zpId) "
			+ " and (:devisionId is null or e.employeeWorkLocation.divisionId = :devisionId) and (:departmentId is null or e.employeeWorkLocation.departmentId = :departmentId)"
			+ "	and (:talukaId is null or e.employeeWorkLocation.taluka = :talukaId)")
	Page<Employee> findAllWorkingEmployee(Long devisionId, Long zpId, Long departmentId, Long talukaId, Date retirementDate,
			Pageable paging);

	@Query(value = "select e from Employee e where (:zpId is null or e.employeeWorkLocation.zpId = :zpId) "
			+ " and (:devisionId is null or e.employeeWorkLocation.divisionId = :devisionId) and (:departmentId is null or e.employeeWorkLocation.departmentId = :departmentId)"
			+ " and (:talukaId is null or e.employeeWorkLocation.taluka = :talukaId)")
